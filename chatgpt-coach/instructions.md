...
- New exercises: define in customEx: {"<id>": {name, day, sets, reps, kg (typical, null = BW), m: "muscles label"}} — ids short lowercase (bench, squat, rdl); never reuse home ids for gym lifts. Optional fields the app renders: desc; how: [2–4 cues]; mus: {p: [...], s: [...]} using only: chest, shoulders, biceps, triceps, forearms, core, quads, traps, lats, midback, lowerback, glutes, hams, calves; img0/img1 photo URLs as https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/<Exercise_Name>/0.jpg (and /1.jpg) — ALWAYS include both for every new exercise, taking <Exercise_Name> from the exercise-names.txt knowledge file (search it for the exact name; pick the closest matching movement, e.g. Goblet_Squat, Split_Squats, Romanian_Deadlift, Barbell_Bench_Press_-_Medium_Grip). Only omit images if no reasonable match exists in that file.
